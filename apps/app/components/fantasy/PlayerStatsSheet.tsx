    ...
    <div className="fixed inset-0 z-50 flex items-end">
      ...
      <div className="relative w-full max-w-sm mx-auto rounded-t-3xl border border-[color:var(--sec-hair)] bg-[color:var(--sec-surface)] px-5 pt-5 pb-[calc(env(safe-area-inset-bottom)+20px)]">
        <div className="flex items-start justify-between mb-3">
          <div>
            <div className="text-sm font-bold text-[color:var(--sec-accent)]">{name}</div>
          ...
          {form != null && Math.abs(form) >= 1 && (
            <span
              className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold ${
                form < 0
                  ? "bg-emerald-500/15 text-[color:var(--sec-good)] border border-[color:var(--sec-accent)]"
                  : "bg-red-500/10 text-[color:var(--sec-bad)] border border-red-500/30"
              }`}
            >
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          {[
            ...
            <div key={label} className="rounded-xl border border-[color:var(--sec-hair)] bg-[color:color-mix(in_srgb,var(--sec-surface)_60%,transparent)] px-2 py-1.5 text-center">
              <div className="text-[13px] font-bold text-[color:var(--sec-text)]">{value}</div>
              <div className="text-[9px] uppercase tracking-wide text-[color:var(--sec-muted)]">{label}</div>
            </div>
          ))}
        </div>

        {courseBest != null && (
          <div className="mb-3 rounded-xl border border-[color:var(--sec-hair)] bg-[color:color-mix(in_srgb,var(--sec-surface)_60%,transparent)] px-3 py-2 text-[11px] text-[color:var(--sec-muted)]">
            Best at this course (recent rounds):{" "}
            <span className="font-bold text-[color:var(--sec-accent)]">{Math.round(courseBest)}</span>
        ...
        {lastFive.length > 0 && (
          <div>
            <div className="text-[9px] uppercase tracking-[0.18em] text-[color:var(--sec-muted)] mb-1.5">
              Recent rounds
            </div>
            <div className="space-y-1">
              {lastFive.map((r, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between rounded-lg border border-[color:var(--sec-hair)] px-2.5 py-1 text-[11px]"
                >
                  ...
                  <span className="text-[color:var(--sec-muted)]">
                    ...
                    <span className="font-bold text-[color:var(--sec-text)]">{Math.round(r.gross18)}</span>
                  </span>
                </div>
              ))}
            </div>
