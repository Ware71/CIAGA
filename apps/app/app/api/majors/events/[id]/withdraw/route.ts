...
// Withdraws the authenticated user from an event, cleaning up their tee time if assigned.
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Check the event allows self-withdrawal
    if ((event as any).allow_self_withdrawal === false) {
      return NextResponse.json(
        { error: "Self-withdrawal is not enabled for this event. Contact the organiser to withdraw." },
        { status: 403 }
      );
    }

    // Cannot withdraw from a live or completed event
    if (event.majors_status === "live" || event.majors_status === "completed") {
      return NextResponse.json(
        { error: "You cannot withdraw from an event that is live or completed." },
        { status: 400 }
      );
    }
    ...
    if (!entry) {
      return NextResponse.json({ error: "You are not entered in this event." }, { status: 404 });
    }

    // Find any tee time the player is in for this event
    ...
    if (roundToDelete) {
      ...
      await supabaseAdmin.from("event_tee_times").delete().eq("id", teeTimeToDelete!);
