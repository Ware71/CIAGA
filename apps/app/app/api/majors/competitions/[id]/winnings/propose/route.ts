...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!competition.group_id) {
      return NextResponse.json({ error: "Competition must be linked to a group." }, { status: 400 });
    }
    ...
    const prizeTable = (competition as any).prize_table as Array<{ position: number; pct: number }> | null;
    if (!prizeTable || prizeTable.length === 0) {
      return NextResponse.json(
        { error: "No prize table configured for this competition. Set one in competition settings." },
        { status: 400 }
      );
    }

    // Calculate total pot from entry fees collected for this competition
    ...
    if (totalPot === 0) {
      return NextResponse.json(
        { error: "No entry fees have been collected for this competition — nothing to distribute." },
        { status: 400 }
      );
    }
