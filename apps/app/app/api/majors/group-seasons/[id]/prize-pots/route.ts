...
async function getGroupSeasonAndAssertAdminOrOwner(groupSeasonId: string, profileId: string) {
  ...
  if (!gs) return { gs: null, groupId: null, authorized: false };
...
// GET /api/majors/group-seasons/[id]/prize-pots
...
// POST /api/majors/group-seasons/[id]/prize-pots
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { profileId } = await getAuthedProfileOrThrow(req);
    const { id } = await params;

    const { gs, groupId, authorized } = await getGroupSeasonAndAssertAdminOrOwner(id, profileId);
    if (!gs) return NextResponse.json({ error: "Group season not found." }, { status: 404 });
