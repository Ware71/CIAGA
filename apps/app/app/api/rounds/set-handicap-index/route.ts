// /app/api/rounds/set-handicap-index/route.ts
...
type Body = {
  ...
  assigned_handicap_index: number | null; // null to clear override
};
  ...
  try {
    ...
    if (round.status !== "draft" && round.status !== "scheduled") {
      return NextResponse.json(
        { error: "Handicap index can only be set before round starts (draft or scheduled)" },
        { status: 400 }
      );
    }
    ...
    if (!isOwner && !isSelf) {
      return NextResponse.json(
        { error: "You can only set handicap index for yourself (or owner can set for anyone)" },
        { status: 403 }
      );
    }
    ...
    const { error: updateErr } = await supabaseAdmin
      .from("round_participants")
      .update({ assigned_handicap_index: body.assigned_handicap_index })
      .eq("id", body.participant_id);
