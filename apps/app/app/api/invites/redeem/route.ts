...
export async function POST(req: Request) {
  try {
    ...
    if (invite.accepted_at || invite.revoked_at) {
      return NextResponse.json({ error: "invite_inactive" }, { status: 410 });
    }
    ...
    if (!profile) return NextResponse.json({ error: "invite_inactive" }, { status: 410 });
    ...
    callbackUrl.searchParams.set("next", "/onboarding/set-password");
    ...
    const { data: linkData, error: linkErr } = await supabaseAdmin.auth.admin.generateLink({
      ...
      options: {
        ...
        data: { profile_id: invite.profile_id, invite_id: invite.id },
      },
    });

    if (linkErr) {
      if (isRateLimited(linkErr)) {
    ...
    if (!actionLink) return NextResponse.json({ error: "server_error" }, { status: 500 });
