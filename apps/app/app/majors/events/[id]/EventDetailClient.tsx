    ...
    <div className="rounded-2xl border border-emerald-900/70 bg-[#0b3b21]/80 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <span className="text-sm font-semibold text-[#7CF0BE]">{timeStr}</span>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="shrink-0 text-center w-14">
          {fixture.status === "completed" && resultLabel ? (
            <span className="text-xs font-bold text-[#7CF0BE]">{resultLabel}</span>
...
function MiniScorecard({
  ...
  function ScoreCell({ hole }: { hole: { hole_number: number; par: number | null } }) {
    ...
    const cls =
      b === "eagle"
        ? `${base} rounded-full bg-[#7CF0BE] text-[#042713]`
        : b === "birdie"
        ? `${base} rounded-full ring-1 ring-[#7CF0BE] text-emerald-50`
        : b === "bogey"
    ...
    return (
      <div className="flex gap-0.5 items-end">
        ...
        <div className="flex flex-col items-center gap-0.5 w-9 border-l border-emerald-900/40 ml-0.5 pl-1">
          ...
          <div className="text-xs font-bold text-[#7CF0BE] tabular-nums">{totals.score || "—"}</div>
        </div>
      </div>
    );
  }
      ...
      <div className="space-y-4">
        ...
        {fantasyNarrative && (
          ...
          >
            <div className="flex items-center justify-between mb-1">
              <div className="text-[9px] uppercase tracking-[0.2em] text-[#7CF0BE]/60">Event preview</div>
        ...
        {(event as any).entry_fee_amount > 0 && (
          <div className="rounded-xl border border-emerald-900/50 bg-[#0b3b21]/60 px-3 py-2.5 flex items-center justify-between">
            ...
            <span className="text-sm font-bold text-[#7CF0BE]">
          ...
          {isAdminOrOwner && allEntrantsComplete && hasFirstPlaceTie && !activePlayoff && (
            <div className="flex gap-2 mb-1.5">
              <button
                ...
                className="flex-1 py-2 rounded-full bg-[#7CF0BE] text-[#042713] text-xs font-semibold"
              >
                Playoff
              </button>
              <button
                ...
                className="flex-1 py-2 rounded-full border border-[#7CF0BE]/50 text-[#7CF0BE] text-xs font-semibold"
              >
                Countback
              </button>
            </div>
            ...
            const inner = (
              <>
                ...
                <div className="text-right shrink-0">
                  <div className="text-xs font-extrabold text-[#7CF0BE]">{mainScoreText}</div>
                  {bracketText ? (
                    ...
                    return (
                      <div key={r.event_round_id ?? i}>
                        ...
                        >
                          ...
                          <div className="text-sm font-bold tabular-nums text-[#7CF0BE] text-right w-12">
          ...
          <>
            ...
            {leagueTable.map((row) => (
              <div key={row.id} className="grid grid-cols-6 items-center rounded-xl border border-emerald-900/50 bg-[#0b3b21]/60 px-3 py-2.5">
                ...
                <span className="text-center text-xs font-bold text-[#7CF0BE]">{row.league_points}</span>
              </div>
            ))}
          </>
        )}
            ...
            <div className="rounded-2xl border border-emerald-900/50 bg-[#0b3b21]/60 px-4 py-3 space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="text-center">
                  ...
                  <div className="text-sm font-bold text-[#7CF0BE]">{currencySymbol}{pot.toFixed(2)}</div>
                </div>
                <div className="text-center">
                  ...
                  <div key={pot.id} className="rounded-xl border border-emerald-900/50 bg-[#0b3b21]/50 overflow-hidden">
                    ...
                    <div className="px-3 py-2.5 space-y-2">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="text-right shrink-0">
                          {pot.is_monetary && (
                            <div className="text-sm font-bold text-[#7CF0BE]">{currencySymbol}{pot.total_pot.toFixed(2)} pot</div>
                          )}
                          ...
                          {proposedDistribution.proposed.map((p, i) => (
                            <div key={i} className="flex items-center justify-between text-[11px]">
                              ...
                              <div className="text-right">
                                <span className="text-[#7CF0BE] font-semibold">
              ...
              <div className="rounded-xl border border-amber-800/40 bg-amber-900/20 px-3 py-3 space-y-3">
                ...
                {proposedWinnings.map((pw) => (
                  <div key={pw.profile_id} className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-[#7CF0BE]">£{pw.amount.toFixed(2)}</span>
                  </div>
                ))}
                <div className="flex gap-2 pt-1">
          ...
          <div className="space-y-2">
            {winnings.map((w) => (
              <div key={w.id} className="flex items-center gap-3 rounded-xl border border-emerald-900/50 bg-[#0b3b21]/60 px-3 py-2.5">
                ...
                <span className="text-sm font-bold text-[#7CF0BE]">£{w.amount.toFixed(2)}</span>
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div className="min-h-[100dvh] pb-[env(safe-area-inset-bottom)] max-w-sm mx-auto">
      ...
      <div className="px-4 mb-4 space-y-2">
        ...
        <h1 className="text-xl font-bold text-[#7CF0BE] leading-tight">{event.name}</h1>
        <div className="flex items-center gap-2 flex-wrap">
