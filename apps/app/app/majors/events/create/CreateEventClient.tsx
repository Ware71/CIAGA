...
export default function CreateEventClient() {
  ...
  const handleEventTemplateSelect = (etId: string) => {
    ...
    setForm((prev) => {
      ...
      if (et) {
        ...
        if (shouldAutoName) {
          name = generateAutoName(et.name, prev.event_date);
  ...
  function generateAutoName(templateName: string, dateStr: string) {
  ...
  const update = (field: keyof FormState, value: string | boolean) =>
    setForm((prev) => {
      ...
      if (field === "event_date" && nameAutoGenerated.current && prev.competition_event_template_id) {
        ...
        if (et) next.name = generateAutoName(et.name, value as string);
      }
        ...
        <div className="space-y-2">
          ...
          {myGroups.length > 0 ? (
            <div className="space-y-1.5">
              <button
                type="button"
                onClick={() => { update("group_id", ""); update("competition_id", ""); setGroupDefaultsApplied(false); setCompetitionEventTemplates([]); setTemplateCompetition(null); }}
                className={`w-full text-left rounded-xl border px-4 py-2 text-sm transition-colors ${
                  form.group_id === ""
                    ? "border-emerald-500 bg-emerald-900/50 text-emerald-50"
    ...
    <div key="step3" className="space-y-4">
      ...
      <div className="rounded-2xl border border-emerald-900/70 bg-[#0b3b21]/80 p-4 space-y-2">
        {[
          ...
          { label: "Season", value: form.standings_contribution !== "event_only" ? "Counting" : "Not counting" },
