  ...
  return (
    <div className="min-h-[100dvh] pb-[env(safe-area-inset-bottom)] px-4 pt-8 max-w-sm mx-auto space-y-5">
      <div className="flex items-center justify-between">
        ...
        <h1 className="text-lg font-semibold tracking-wide text-[color:var(--sec-accent)]">Majors Profile</h1>
        <div className="w-14" />
      </div>
        ...
        <>
          ...
          <div className="flex items-center gap-4">
            {data.profile.avatar_url ? (
              <img src={data.profile.avatar_url} alt="" className="h-14 w-14 rounded-full object-cover border border-[color:var(--sec-hair)]" loading="lazy" decoding="async" />
            ) : (
              <div className="h-14 w-14 rounded-full bg-[color:var(--sec-surface)] grid place-items-center text-xl font-bold text-[color:var(--sec-text-2)]">
                {data.profile.name?.slice(0, 2).toUpperCase() ?? "?"}
              </div>
            )}
            <div>
              <div className="text-base font-extrabold text-[color:var(--sec-text)]">{data.profile.name ?? "Player"}</div>
              <div className="text-[11px] text-[color:var(--sec-muted)]">CIAGA Majors</div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-[color:var(--sec-hair)] bg-[color:color-mix(in_srgb,var(--sec-surface)_80%,transparent)] p-4">
            <div className="text-[10px] uppercase tracking-[0.18em] text-[color:var(--sec-muted)] mb-3">
              This Season
            </div>
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={stat.label} className="flex items-center justify-between">
                  <span className="text-[12px] text-[color:var(--sec-muted)]">{stat.label}</span>
                  <span className="text-[12px] font-extrabold text-[color:var(--sec-text)]">{stat.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {data.recent_results.length > 0 && (
            <section className="space-y-2">
              <h2 className="text-[10px] uppercase tracking-[0.18em] text-[color:var(--sec-muted)]">
                Recent Results
              </h2>
              {data.recent_results.map((item) => (
                <button
                  ...
                  className="w-full text-left flex items-center justify-between rounded-xl border border-[color:var(--sec-hair)] bg-[color:color-mix(in_srgb,var(--sec-surface)_60%,transparent)] px-4 py-3 hover:border-[color:var(--sec-line)]"
                >
                  ...
                  <div className="shrink-0 text-right ml-3">
                    <div className="text-sm font-extrabold text-[color:var(--sec-accent)]">
          ...
          {data.group_memberships.length > 0 && (
            <section className="space-y-2 pb-8">
              ...
              {data.group_memberships.map((m) => (
                ...
                >
                  <div>
                    <div className="text-sm font-semibold text-[color:var(--sec-text)]">{m.group.name}</div>
                    <div className="text-[10px] text-[color:var(--sec-muted)] capitalize">
                  ...
                  {m.standing && (
                    <div className="text-right">
                      ...
                      <div className="text-[10px] text-[color:var(--sec-muted)]">{m.standing.season_points} pts</div>
                    </div>
                  )}
                </button>
              ))}
