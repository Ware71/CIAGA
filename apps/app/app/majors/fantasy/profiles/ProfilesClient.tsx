...
function confidenceBadge(confidence: ProfileRow["confidence"]): string {
  if (confidence === "high") return "text-[color:var(--sec-good)] border-[color:var(--sec-line)]";
  if (confidence === "medium") return "text-amber-300/80 border-amber-800/40";
  return "text-[color:var(--sec-muted)] border-[color:var(--sec-hair)]";
}
  ...
  return (
    <div className="min-h-[100dvh] pb-[env(safe-area-inset-bottom)] max-w-sm mx-auto">
      <div className="px-4 pt-8 flex items-center justify-between mb-6">
        ...
        <h1 className="text-lg font-bold tracking-wide text-[color:var(--sec-accent)]">Profiles</h1>
        <div className="w-12" />
      </div>
        ...
        <div className="px-4 space-y-4 pb-12">
          ...
          {loadingRows ? (
            ...
            <div className="rounded-2xl border border-[color:var(--sec-hair)] bg-[color:color-mix(in_srgb,var(--sec-surface)_80%,transparent)] px-4 py-6 text-center">
              <div className="text-sm text-[color:var(--sec-muted)]">No profiles yet</div>
              ...
              {rows.map((row) => {
                ...
                return (
                  <button
                    ...
                    className="w-full text-left rounded-2xl border border-[color:var(--sec-hair)] bg-[color:color-mix(in_srgb,var(--sec-surface)_70%,transparent)] px-3 py-2.5"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-[13px] font-semibold text-[color:var(--sec-text)] truncate">{name}</div>
                    ...
                    {open && (
                      <div className="mt-2 pt-2 border-t border-[color:var(--sec-hair)] grid grid-cols-3 gap-y-1.5 text-center">
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div className="text-[11px] font-bold text-[color:var(--sec-text)]">{value}</div>
      <div className="text-[9px] text-[color:var(--sec-muted)]">{label}</div>
    </div>
  );
}
