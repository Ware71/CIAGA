  ...
  return (
    <div className="min-h-[100dvh] pb-[env(safe-area-inset-bottom)] max-w-sm mx-auto">
      ...
      <div className="px-4 mb-5 space-y-1">
        <h1 className="text-xl font-bold text-[color:var(--sec-accent)] leading-tight">{templateName}</h1>
        <p className="text-[11px] text-[color:var(--sec-muted)]">All-time history</p>
      </div>
        ...
        <div className="px-4 mb-5">
          <div className="rounded-2xl border border-[color:var(--sec-hair)] bg-[color:color-mix(in_srgb,var(--sec-surface)_80%,transparent)] p-4">
            ...
            {(viewerStats.best_net_score != null || viewerStats.avg_net_score != null) && (
              <div className="mt-3 text-[10px] text-[color:var(--sec-muted)] text-center">
        ...
        {history.length === 0 ? (
          ...
          history.map((row) => (
            <button
              ...
              className="w-full text-left rounded-2xl border border-[color:var(--sec-hair)] bg-[color:color-mix(in_srgb,var(--sec-surface)_80%,transparent)] p-3 space-y-2 hover:border-[color:var(--sec-line)] transition-colors"
            >
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold text-[color:var(--sec-accent)]">{row.season_year}</span>
                <span className="text-[10px] text-[color:var(--sec-muted)]">{row.field_size} players</span>
              </div>
              ...
              {row.winner ? (
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    <div className="text-xs font-semibold text-[color:var(--sec-text)] truncate">{row.winner.name ?? "Unknown"}</div>
                    <div className="text-[10px] text-[color:var(--sec-muted)]">Winner{row.winning_score_summary ? ` · ${row.winning_score_summary}` : ""}</div>
                  </div>
                </div>
              ) : (
                <div className="text-xs text-[color:var(--sec-muted)] italic">No result recorded</div>
              )}

              {row.runner_up && (
                <div className="flex items-center gap-2 opacity-60">
                  <div className="h-5 w-5 rounded-full bg-[color:var(--sec-surface)] grid place-items-center text-[8px] font-bold text-[color:var(--sec-text-2)]">
                    {(row.runner_up.name ?? "?").slice(0, 2).toUpperCase()}
                  </div>
                  <span className="text-[11px] text-[color:var(--sec-muted)] truncate">Runner-up: {row.runner_up.name ?? "Unknown"}</span>
                </div>
              )}

              {/* Viewer's own result for this year */}
              {row.viewer_entry && (
                <div className="flex items-center gap-2 pt-1 border-t border-[color:var(--sec-hair)]">
                  <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-[color:var(--sec-surface)] text-[color:var(--sec-good)] border border-[color:var(--sec-hair)] shrink-0">
                    You
                  </span>
