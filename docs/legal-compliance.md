...
| **In-app reporting (OSA)** | Report any post or comment from the `···` menu: ten fixed reason codes (spam, harassment, hate, violence, sexual, self-harm, illegal, impersonation, copyright, other) plus optional free text, and an on-screen confirmation that the report was received. One report per person per item. Also "Hide this post" (per-device, instant, no admin needed) and self-delete of your own posts | `apps/app/lib/social/report.ts`, `apps/app/components/social/{ReportSheet,OverflowMenu}.tsx`, `apps/app/lib/social/hidden.ts`, `apps/app/app/api/reports/route.ts`, `DELETE apps/app/app/api/feed/[id]/route.ts` | Live |
| **Moderation queue + takedown (OSA)** | Admin-only queue at `/admin/reports` with Open/Reviewing/Actioned/Dismissed, a preview of the reported content and a link to it in context, and Hide / Remove / Restore. Targets with 3+ distinct reporters sort to the top and are flagged — we alert rather than auto-hide, because in a society this size three friends should not be able to silence anyone. Every decision writes an audit row | `apps/app/app/admin/reports/page.tsx`, `apps/app/app/api/admin/{reports,reports/[id],moderation}/route.ts`, `setContentVisibility()` in `apps/app/lib/feed/commands.ts` | Live, untested |
| **Data minimisation on upload** | Post photos are re-encoded client-side through a canvas before upload, which strips all EXIF — including the GPS coordinates iPhones embed. Without this, the exact location of every photo would land in a public bucket readable by anyone with the URL | `apps/app/lib/media/compressImage.ts`, bucket + RLS in `supabase/migrations/20260903000000_post_images_bucket.sql` | Live, untested |
...
| Schema support (moderation) | `feed_reports.{reason_code,status,resolved_by,resolved_at,resolution_note}` + one-report-per-reporter unique index; `feed_moderation_actions` audit table (service-role only). Note `feed_reports` rows are still hard-deleted on account deletion — the *decision* record in `feed_moderation_actions` survives (its `report_id` FK nulls), so the audit trail outlives the reporter | `supabase/migrations/20260903000002_feed_moderation.sql` | Applied (staging) — **not yet prod** |
