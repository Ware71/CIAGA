-- Standardise playing handicap allowance rounding to ROUND() (WHS standard).
--
-- Migrations 003–005 introduced FLOOR() for the event allowance_pct computation:
--   FLOOR(course_handicap_used * allowance_pct / 100)
--
-- The original ciaga_resolve_playing_handicap (20260216023331) uses ROUND() — nearest
-- integer — which matches WHS: "playing handicap = course handicap × allowance %, rounded
-- to the nearest integer". Using FLOOR instead produces different results depending on
-- which code path computed playing_handicap_used, causing players in the same round to
-- receive inconsistent strokes (e.g. CH 24 × 95% = 22.8 → ROUND = 23 vs FLOOR = 22).
--
-- This migration replaces FLOOR with ROUND in:
--   1. ciaga_persist_playing_handicaps Step 3 (event allowance_pct branch)
--   2. ciaga_compute_event_leaderboard  — all 4 allowance_pct CASE expressions
--      (stab_pts hv, submitted submitted_hcp SUM, live_rounds course_hcp, live_rounds hv)
-- Other FLOOR usages (base-strokes per-hole allocation, proportional live handicap) are
-- unrelated to allowance rounding and are left unchanged.

-- ============================================================
-- 1. ciaga_persist_playing_handicaps: FLOOR → ROUND in Step 3
-- ============================================================
...
BEGIN
  ...
  -- WHS standard: allowance_pct result is rounded to nearest integer (ROUND, not FLOOR).
  UPDATE public.round_participants rp
...
-- ============================================================
-- 2. ciaga_compute_event_leaderboard: FLOOR → ROUND in 4 places
-- ============================================================
...
-- ============================================================
-- 3. Backfill: re-run ciaga_persist_playing_handicaps for all
--    event rounds with allowance_pct mode so stored
--    playing_handicap_used values use ROUND instead of FLOOR.
-- ============================================================
...
-- ============================================================
-- 4. Recompute all event leaderboards.
-- ============================================================
