-- ============================================================
-- Fix ciaga_event_round_leaderboard: rounds have no 'cancelled' status.
--
-- 20260825000005 filtered the field with `r.status <> 'cancelled'`. The
-- round_status enum is (draft, scheduled, starting, live, finished) — there is
-- no cancelled member, so comparing to it raises
--   invalid input value for enum round_status: "cancelled"
-- the moment the function is called. Creating it succeeded, so this only
-- surfaced at runtime.
--
-- A cancelled ROUND is not a thing; a cancelled EVENT ROUND is, and that is
-- already handled by the caller (the round tab is built from event_rounds).
-- ============================================================
