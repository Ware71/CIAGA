-- ============================================================
-- Fix: ciaga_compute_competition_leaderboard referenced agg.position
-- inside the same SELECT level where position is computed as a window
-- function — PostgreSQL does not allow this. Solution: add a middle
-- subquery ("ranked") that computes position first, so the outer SELECT
-- can reference ranked.position for the points_earned CASE.
-- ============================================================
...
BEGIN
  ...
  SELECT
    ...
    -- position is now available from the middle subquery
    CASE
    ...
    NOW() AS computed_at
  FROM (
    -- Middle subquery: compute position (RANK) and to_par so the outer
    -- SELECT can reference them by name.
    SELECT
      agg.profile_id,
      agg.gross_score,
      agg.net_score,
