-- ============================================================
-- Event results must use ACTUAL gross, not the WHS adjusted gross.
--
-- handicap_round_results.adjusted_gross_score applies net-double-bogey
-- capping, which exists purely for HANDICAPPING (it stays untouched
-- there). Event leaderboards/results for stroke play must rank on the
-- player's real strokes: actual gross − playing handicap.
--
-- The `submitted` CTE of ciaga_compute_event_leaderboard summed
-- hrr.adjusted_gross_score, so a player's event score could drop the
-- moment their round finished (live scoring + the stableford path both
-- already sum raw strokes from round_score_events). Same issue in the
-- full-rounds branch of ciaga_get_frozen_leaderboard (the dynamic
-- fallback used when freeze snapshots are missing).
--
-- Fix: sum the latest score event per hole; fall back to the adjusted
-- gross only for legacy/manual submissions with no hole-level data
-- (e.g. season-import rounds).
-- ============================================================

-- ── 1. ciaga_compute_event_leaderboard ───────────────────────
-- Base: 20260610000005_fix_tied_position_rank.sql. Only the `submitted`
-- CTE changes (raw-gross lateral + COALESCE).
  ...
  FROM (
    ...
    FROM (
      ...
      submitted AS (
        ...
        LEFT JOIN LATERAL (
          SELECT SUM(ls.strokes)::integer AS gross
          FROM (
            SELECT DISTINCT ON (rse.hole_number)
              rse.hole_number,
              rse.strokes
  ...
  sub_details AS (
    ...
    LEFT JOIN LATERAL (
      ...
      FROM (
        ...
        FROM round_score_events rse
        WHERE rse.round_id = rs.round_id
          AND rse.participant_id = rp.id
        ORDER BY rse.hole_number, rse.created_at DESC
      ) ls
    ) raw ON true
...
-- ── 3. Recompute currently-live events ────────────────────────
-- (Completed events are not mass-rewritten; the Invitational is repaired
-- by a targeted one-off script.)
DO $$
DECLARE v_id uuid;
BEGIN
  FOR v_id IN SELECT id FROM public.events WHERE majors_status = 'live' LOOP
