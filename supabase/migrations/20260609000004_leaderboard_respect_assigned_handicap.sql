-- Fix: ciaga_compute_event_leaderboard now respects assigned_playing_handicap overrides
-- for allowance_pct events.
--
-- Migration 20260609000002 added the allowance_pct calculation but always applied the
-- formula FLOOR(course_handicap_used * allowance_pct / 100), silently ignoring any
-- manual assigned_playing_handicap set by an organiser. This migration adds the
-- assigned_playing_handicap check as the highest-priority branch in all four handicap
-- CASE expressions (stab_pts hv, submitted_hcp SUM, live_rounds course_hcp, live_rounds hv).
--
-- Priority order now matches ciaga_persist_playing_handicaps (migration 20260609000003):
--   1. assigned_playing_handicap (manual override)
--   2. allowance_pct formula: FLOOR(course_handicap_used * allowance_pct / 100)
--   3. COALESCE(playing_handicap_used, course_handicap_used, 0)  (round default)
...
-- Recompute all event leaderboards so existing entries pick up the corrected handicap priority.
DO $$
DECLARE
  v_id uuid;
BEGIN
