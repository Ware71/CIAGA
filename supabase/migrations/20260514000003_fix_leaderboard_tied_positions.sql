-- ============================================================
-- Fix competition leaderboard:
--   1. Use RANK() instead of ROW_NUMBER() so tied players share
--      the same position and receive equal points_earned.
--   2. Include all registered participants (from tee times) in
--      the leaderboard, not just those who have scored.
--      Pending players appear at the bottom with NULL scores.
-- Also update ciaga_compute_season_standings to use RANK().
-- ============================================================
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      CASE
        WHEN inner_agg.net_score IS NULL THEN NULL
        ELSE RANK() OVER (
          ORDER BY
...
-- ── Update season standings to use RANK() ────────────────────
...
BEGIN
  ...
  SELECT
    ...
    -- Use RANK() so tied season-point totals share the same standing position
    RANK() OVER (
      ORDER BY
  ...
  WHERE c.season_id = p_season_id
    ...
    AND agg.points_earned IS NOT NULL
