-- Expose competition_tee_time_id in get_round_detail_snapshot so RoundDetailClient
-- can resolve which competition a scorecard belongs to (for live standings).
  ...
  SELECT jsonb_build_object(
    'round', (
      SELECT jsonb_build_object(
        ...
        'course_name', c.name,
        'competition_tee_time_id', r.competition_tee_time_id
      )
      FROM rounds r
...
COMMENT ON FUNCTION public.get_round_detail_snapshot IS
  'Returns all data needed by the round detail page in a single call.
   Includes: round meta (incl. competition_tee_time_id), participants (with resolved handicaps),
   participant extras (playing_handicap_used, team_id), teams (with playing_handicap_used),
   tee snapshot, hole snapshots, current scores, and hole states.';
