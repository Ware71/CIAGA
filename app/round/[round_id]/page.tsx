            ...
            <div className="rounded-2xl border border-emerald-900/70 bg-[#0b3b21]/70 overflow-hidden">
              <div className="w-full">
                <div className="w-full" style={{ display: "grid", gridTemplateColumns: portraitCols }}>
                      ...
                      >
                        {compactPlayers ? (
                          ...
                          <div className="flex items-center gap-1 min-w-0">
                            ...
                            <div className="text-[10px] text-emerald-50 truncate min-w-0">{name}</div>
                          </div>
                        )}
                      </div>
                    );
                  ...
                  {holesList.flatMap((h) => {
                    ...
                    participants.forEach((p) => {
                      ...
                      nodes.push(
                        <button
                          key={`sc-${key}`}
                          className={`h-9 border-b border-r border-emerald-900/60 flex items-center justify-center font-semibold tabular-nums
                    ...
                    if (h.hole_number === 9) {
                      ...
                      participants.forEach((p) => {
                        nodes.push(
                          <div
                            key={`out-mid-${p.id}`}
                            className="h-9 border-b border-r border-emerald-900/60 flex items-center justify-center font-bold tabular-nums text-[12px] bg-[#0b3b21]/35 text-emerald-50"
                          >
                            {totals[p.id]?.out ?? 0}
                          </div>
                        );
                  ...
                  {/* Totals rows at end: OUT / IN / TOT (with Par + Yds) */}
                  {(["OUT", "IN", "TOT"] as const).flatMap((label) => {
                    ...
                    participants.forEach((p) => {
                      ...
                      nodes.push(
                        <div
                          key={`tot-${label}-${p.id}`}
                          className={`h-9 border-b border-r border-emerald-900/60 flex items-center justify-center font-bold tabular-nums ${
                            compactPlayers ? "text-[12px]" : "text-[13px]"
                          ...
                          {landscapePlan.map((c, idx) => {
                            if (c.kind === "hole") {
                              ...
                              return (
                                <button
                                  ...
                                  className={`h-10 border-r border-emerald-900/60 flex items-center justify-center font-semibold tabular-nums text-[13px]
                            ...
                            return (
                              <div
                                ...
                                className={`h-10 border-r border-emerald-900/60 flex items-center justify-center font-bold tabular-nums text-[12px]
          ...
          <div className="p-3 border-b border-emerald-900/60 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[11px] text-emerald-100/70">
                  Hole {holeNumber} · Par {holeMeta?.par ?? "–"} · Yds {holeMeta?.yardage ?? "–"} · SI{" "}
                  {holeMeta?.stroke_index ?? "–"}
